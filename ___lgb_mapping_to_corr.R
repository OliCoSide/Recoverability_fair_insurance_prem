the_mapping_to_corr_lightgbm_fun <- function(name, selectA=0:1){
  ...
  if(length(selectA) == 2){
    ...
    best_fn <- best_lgb_A0[[name]]
  } else{
    best_fn <- best_lgb_A1[[name]]
  }
  
  ## All the epsilon_y and best estimates and corrective
  beg_read <- Sys.time()
  eps_y <- jsonlite::fromJSON(paste0('transported/', name, '_', selection_label,
                                     '_bary_mapping.json'))
  to_train <- data.frame('SB' = best_fn$preds$train,
                         'D' = (sims[[name]] %>% filter(A %in% selectA))$D,
                         'SC' = eps_y$train)
  to_valid <- data.frame('SB' = best_fn$preds$valid,
                         'D' = (valid[[name]] %>% filter(A %in% selectA))$D,
                         'SC' = eps_y$valid)
  
  cat("Data import:", Sys.time() - beg_read, " sec. \n")
  beg_treat <- Sys.time()
  
  trn <- to_train %>% dplyr::select(SB, D) %>% mutate(D = factor(D, levels = 0:1))
  vld <- to_valid %>% dplyr::select(SB, D) %>% mutate(D = factor(D, levels = 0:1))
