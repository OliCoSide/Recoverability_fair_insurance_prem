the_best_estimate_lightgbm_fun <- function(name, selectA=0:1){
  ...
  trn <- train_data %>% dplyr::select(X, D) %>% mutate(D = factor(D, levels = 0:1))
  vld <- valid_data %>% dplyr::select(X, D) %>% mutate(D = factor(D, levels = 0:1))
  tst <- test_data %>% dplyr::select(X, D) %>% mutate(D = factor(D, levels = 0:1))
  ...
  ## CAREFUL I THINK YOU'RE MISSING A -1 HERE CAUSE THE IDs START AT 0 IN THAT FUNCTION
  ...
  preds %>% jsonlite::toJSON(., pretty = TRUE) %>%
    write(paste0('preds/', name, '_', selection_label, '_best_estimate.json'))
